...
const normalizePercent = (value: any): number => {
  ...
  return raw.includes("%") ? n : Math.abs(n) <= 1 ? n * 100 : n;
};


const SCRIPT_URL =
  (process as any).env?.REACT_APP_PREMIUM_SCRIPT_URL ||
  "https://script.google.com/macros/s/AKfycbxteerJmuIYrbnErKw9QKLs1wRdQXQ053gaDeQX7w55_ZDBJJNk8c1kMOFfWiUfgcStdA/exec";
    ...
    () =>
      (data.holdings || []).map((h: any) => {
        ...
        return {
          ...
          costValue,
          gl,
          glPct: normalizePercent(costValue > 0 ? (gl / costValue) * 100 : pct(h.glPct ?? h.unrealizedPLPercent)),
        };
      }),
    [data.holdings]
  );
